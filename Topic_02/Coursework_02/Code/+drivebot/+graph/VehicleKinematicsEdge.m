...
classdef VehicleKinematicsEdge < g2o.core.BaseBinaryEdge
    ...
    methods(Access = public)
        ...
        function initialize(VehicleKinematicsEdge_obj)
            ...
            M = VehicleKinematicsEdge_obj.dT * [c -s 0;
                s c 0;
                0 0 1];
            
            % Compute the posterior assming no noise
            VehicleKinematicsEdge_obj.edgeVertices{2}.x = VehicleKinematicsEdge_obj.edgeVertices{1}.x + M * VehicleKinematicsEdge_obj.z;

            % Wrap the heading to -pi to pi
            VehicleKinematicsEdge_obj.edgeVertices{2}.x(3) = g2o.stuff.normalize_theta(VehicleKinematicsEdge_obj.edgeVertices{2}.x(3));
        ...
        function computeError(VehicleKinematicsEdge_obj)
            ...
            dx(3) = g2o.stuff.normalize_theta(dx(3)); % wrap in the range -pi to pi.
        ...
        function linearizeOplus(VehicleKinematicsEdge_obj)
            ...
            c = cos(priorX(3));
            s = sin(priorX(3));
            dx = VehicleKinematicsEdge_obj.edgeVertices{2}.x - priorX; 
            dx(3) = g2o.stuff.normalize_theta(dx(3)); % wrap in the range -pi to pi.
            Mi = [c s 0;
                -s c 0;
                0 0 1];
            ...
            % for k+1 side
            VehicleKinematicsEdge_obj.J{2} = Mi; % the Jacobian wrt x_k+1 is inverse of M/dT

            
            % the jacobian wrt x_k
